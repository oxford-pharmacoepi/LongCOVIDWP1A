...
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (4244350,4043565)
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,-90,P.END_DATE) AND A.START_DATE <= DATEADD(day,-14,P.END_DATE) ) cc on p.person_id = cc.person_id and p.event_id = cc.event_id
