...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (37016663,140821,19126203,4186213,4024566,255348)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (140821,255348)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (315830,4306496,46270370,4312593,4090870,4249605,43531610,4300746,4196986,4215494,4344370,36674193,37110783,4331946,4035960,46273539,4292397,761827,4091963,256717,4326264,36676582,4019443,4164440,4005765,134681,258828,45768908,40405599,4024992,4093991,4208994,37109663,36674719,36717006,36713056,4316217)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (315830,4306496,46270370,4312593,4090870,4249605,43531610,4300746,4196986,4215494,4344370,36674193,37110783,4035960,46273539,4292397,761827,4091963,256717,4326264,36676582,4019443,4164440,4005765,134681,258828,45768908,40405599,4024992,4093991,4208994,37109663,36674719,36717006,36713056,4316217)
  and c.invalid_reason is null
...
AND YEAR(C.measurement_date) - P.year_of_birth >= 18
...
JOIN @cdm_database_schema.PERSON P on C.person_id = P.person_id
WHERE YEAR(C.observation_date) - P.year_of_birth >= 18
...
) A on A.person_id = P.person_id  AND A.START_DATE >= P.OP_START_DATE AND A.START_DATE <= P.OP_END_DATE AND A.START_DATE >= DATEADD(day,90,P.END_DATE) AND A.START_DATE <= P.OP_END_DATE ) cc 
