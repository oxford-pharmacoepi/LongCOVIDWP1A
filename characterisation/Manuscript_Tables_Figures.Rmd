...
for (i in 1:length(files)){
  assign(files[i], read.csv(paste0(folder, "/", files[i], ".csv"), header = TRUE, sep = ","))
} 
...
plot <- RR_data  %>%
    ...
    scale_y_discrete(expand=c(0,0), position = "left") +
     scale_x_log10(breaks = c(0.1, 0.2, 0.4, 0.6, 0.8, 1),
                   labels = c("0.1", "0.2", "0.4","0.6", "0.8", "1"),
                  limits = c(0.035, 1.2)
                  ) +  
    scale_colour_manual(values = c(col1, col2)) +
    geom_vline(
...
Incidence_rates_Covid_LongCov_SIDIAP$time <- NULL
Incidence_rates_Covid_LongCov_CPRD <- Incidence_rates_Covid_LongCov_CPRD %>%
  rename(ir_100000_pys = incidence_100000_pys,
...
data_IR_fig90 <- IR_data %>%
  ...
  select(incidence_start_date, n_persons, n_events, person_years, ir_100000_pys,  ir_100000_pys_95CI_lower,  ir_100000_pys_95CI_upper, age_group, sex, cohort, database) %>%
  rbind(Proportion_data %>%
          mutate(person_years = person_days/365) %>%
          select(-person_days) %>%
          filter(cohort=="COVID-19 infection") %>%
          mutate(age_group = ifelse(age_group == "<=34", "18;34",
                              ifelse(age_group == "35-49", "35;49", 
                              ifelse(age_group == "50-64", "50;64",
                              ifelse(age_group == "65-79", "65;79",
                              ifelse(age_group == ">=80", "80;150",
                              ifelse(age_group == "Overall", "Overall", 
                                     NA))))))) %>%
          mutate(cohort = "Long COVID-19 among those with COVID-19, 90 days")
  ) %>%
  mutate(age_group = ifelse(age_group == "18;150", "Overall", 
                            ifelse(age_group == "80;150", "\u226580", age_group))) %>%
  mutate(age_group = factor(age_group,
                            levels = c("Overall", "18;34", "35;49", "50;64", "65;79", "\u226580"))) %>%
...
### Figure 2: RR of long COVID symptoms: COVID-19 infection vs first SARS-CoV-2 negative test - 90 days
```{r, fig.dim = c(8, 8), echo = FALSE}
...
fig_IR_age_sex_CPRD <-  plot_IR(data = data_IR_fig90 %>% filter(database == "CPRD"),
                                     age_to_filter = c(
                                       "18;34",
                                             "35;49",
                                             "50;64",
...
```{r}
table_RR_90 <- get_RR(sidiap_data = RR_new_covid_tested_negative_all_90days_SIDIAP,
                  cprd_data = RR_new_covid_tested_negative_all_90days_CPRD)
table_RR_90
```
...
```{r}
table_RR_28 <- get_RR(sidiap_data = RR_new_covid_tested_negative_all_28days_SIDIAP,
                  cprd_data = RR_new_covid_tested_negative_all_28days_CPRD)
table_RR_28
```
