...
for(i in 1:2) {
# tidy up 
  cur_column <- summary.characteristics.confirmed[, i]
  cur_column <- str_extract(cur_column, '[0-9.]+\\b') %>% 
                as.numeric() 
  cur_column <-nice.num(cur_column)
  # add back in
 summary.characteristics.confirmed[, i] <- str_replace(string=summary.characteristics.confirmed[, i],
                              pattern='[0-9.]+\\b', 
                              replacement=cur_column)    
}
...
for(i in 1:2) {
# tidy up 
  cur_column <- summary.characteristics_symptoms[, i]
  cur_column <- str_extract(cur_column, '[0-9.]+\\b') %>% 
                as.numeric() 
  cur_column <-nice.num(cur_column)
  # add back in
summary.characteristics_symptoms[, i] <- str_replace(string=summary.characteristics_symptoms[, i],
                              pattern='[0-9.]+\\b', 
                              replacement=cur_column)    
}
