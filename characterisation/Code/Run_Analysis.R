...
# cohortNames
jsonCohortName <- paste0(cohortStem, "_json")
attritionCohortName <- paste0(cohortStem, "_attrition_cohorts")

# get cohort Id
getCohortId <- function(x, name) {
  cohortSet(cdm) %>%
...
cdm <- generateCohortSet(cdm, cohorts, jsonCohortName)

new_infection_id            <- getCohortId(cdm[jsonCohortName], "New_Infection") 
first_infection_id          <- getCohortId(cdm[jsonCohortName], "First_infection")
reinfection_id              <- getCohortId(cdm[jsonCohortName], "Reinfection") 
tested_negative_all_id      <- getCohortId(cdm[jsonCohortName], "Tested_Negative_all_events") 
tested_negative_earliest_id <- getCohortId(cdm[jsonCohortName], "Tested negative, earliest") 
long_covid_id               <- getCohortId(cdm[jsonCohortName], "Long COVID-19 symptoms (with test start date)")
long_covid_symptom_id       <- getCohortId(cdm[jsonCohortName], "Long COVID-19 symptoms (with symptom start date)") 
...
covid_ids <- getCohortId(cdm[attritionCohortName], "New_Infection") 
control_id <- getCohortId(cdm[attritionCohortName], "Tested_Negative_all_events")
covid_censoring_id <- getCohortId(cdm[attritionCohortName], "LongCov-ER_Tested_Positive_and_diagnoses_censoring")
influenza_id <- getCohortId(cdm[attritionCohortName], "LongCov-ER_Influenza_dx_positive_test_censoring")
symptoms_ids <- getCohortId(cdm[attritionCohortName], symptoms)

target <- cdm[[attritionCohortName]] %>%
  filter(cohort_definition_id == 1) %>%
  addDemographics(cdm, futureObservation = FALSE) %>%
  timeToCohort(cdm, attritionCohortName, 1, order = "last", window = c(-Inf, -1), nameStyle = "previous_covid") %>%
  timeToCohort(cdm, jsonCohortName, influenza_id, order = "last", window = c(-Inf, 0), nameStyle = "previous_flu") 

apendAttritionLine <- function(attrition = NULL, x, reason) {
  if (is.null(attrition)) {
    reasonId <- 1
  } else {
    reasonId <- attrition$reason_id %>% max()
  }
  attritionLine <- x %>%
    ...
    dplyr::mutate(reason = .env$reason, reason_id = .env$reasonId + 1)
  if (is.null(attrition)) {
    attrition <- attritionLine %>% 
      dplyr::mutate(excluded_records = 0, excluded_subjects = 0)
  } else {
    attrition <- attrition %>%
      dplyr::union_all(
        attritionLine %>%
